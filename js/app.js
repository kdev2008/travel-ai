...
async function selectCountry(input, silent) {
  ...
  try {
    ...
    if (token !== countryToken) return;
    const latest = await api('countryBundle', ctx({ country: c.name.common, countryCode: c.cca2, passport: profile.passport || profile.home }));
...
async function openCity(city) {
  ...
  const bundleDetail = currentCountryBundle && currentCountryBundle.cityDetails && currentCountryBundle.cityDetails[String(city).trim().toLowerCase().replace(/[^a-z0-9]+/g,'')];
