/* Travel AI — application logic
   BUILD 3.1 — traveller platform, single file.
...
function api(action, params) {
  if (!CFG.API_URL) {
    return Promise.reject(new Error('The AI backend is not connected. Add your Apps Script /exec URL to js/config.js as API_URL.'));
  }
  let u;
  ...
  return fetchTimeout(u.toString(), { cache: 'no-store', redirect: 'follow' }, CFG.REQUEST_TIMEOUT_MS || 75000)
    ...
    .catch(err => {
      if (err && err.name === 'AbortError') throw new Error('That took too long. Try again.');
...
/* Traveller context sent with every AI call. */
function ctx(extra) {
...
function openScan(name, code) {
  ...
  scanFailsafe = setTimeout(hideScan, 20000);
}
function hideScan() {
...
async function selectCountry(input, silent) {
  ...
  try {
    ...
    if (!silent) { setScan(18, 'Country intelligence...'); scanMark('scanGeo'); }
    renderCountry();
    showView('country');
    if (!silent) await wait(120);

    if (!silent) setScan(40, 'Currency and weather...');
    await Promise.allSettled([
      loadRate().then(() => scanMark('scanFx')),
      loadWeather().then(() => scanMark('scanWx'))
    ]);
    if (token !== countryToken) return;

    if (!silent) setScan(66, 'Visa position for your passport...');
    loadVisaBrief().then(() => scanMark('scanVisa'));

    if (!silent) setScan(85, 'Traveller context...');
    loadCities().then(() => scanMark('scanAi'));
    ...
    if (!silent) setTimeout(() => {
      const o = $('#scanOverlay');
      if (o && o.classList.contains('open') && token === countryToken) hideScan();
    }, 6000);
  }
}
...
async function loadCities(force) {
  ...
  if (!CFG.API_URL) {
    grid.innerHTML = '<div class="brief-empty">Connect the AI backend to map cities. ' +
  ...
  try {
    ...
    grid.innerHTML = list.map((c, i) =>
      ...
      '<div class="city-top"><div><b>' + esc(c.name) + '</b>' +
      '<small class="reg">' + esc(c.region || '') + '</small></div>' +
      (c.score ? '<div class="city-score">' + esc(c.score) + '</div>' : '') + '</div>' +
      '<p>' + esc(c.famousFor || '') + '</p>' +
      '<div class="city-tags">' + (c.tags || []).slice(0, 3).map(t => '<em>' + esc(t) + '</em>').join('') + '</div>' +
      '<div class="city-meta"><span>◷ ' + esc(c.days || '—') + '</span>' +
      '<span>❄ ' + esc(c.bestTime || '—') + '</span>' +
      (c.bestFor ? '<span>★ ' + esc(c.bestFor) + '</span>' : '') + '</div>' +
      '<div class="city-go">EXPLORE ▸</div></article>').join('');
  ...
  try {
    ...
    if (dailyTotal) {
      html += '<section class="dsec"><header><span class="dsec-kicker">DAILY COST</span>' +
        ...
        '<div class="cost-bars">' + costBars(daily, localCur) + '</div>' +
...
async function loadCosts(force) {
  ...
  try {
    ...
    '<div class="rec-total" style="margin-top:14px"><span class="badge badge-est">Estimated fares</span>' +
    '<small style="color:#8ba7b8">Flights are AI estimates until a fare API is connected.</small></div>' +
...
async function loadVisaBrief(force) {
  ...
  try {
    ...
    const badge = $('#countryVisaBadge');
    if (badge) { badge.textContent = 'Visa: unavailable'; badge.className = 'visa-badge visa-unk'; }
  }
}
...
async function runFinder() {
  ...
  try {
    ...
    out.innerHTML = picks.map((p, i) => recCard(p, i, cur, days)).join('') +
      '<div class="disclaimer">Flight figures are AI estimates, not live fares. ' +
...
function recCard(p, i, cur, days) {
  ...
  return '<article class="rec" style="--i:' + i + '">' +
    ...
    '<span class="badge badge-est">Estimated fares</span></div>' +
    '</div>' +
...
async function loadMonthPicks(force) {
  ...
  if (!CFG.API_URL) {
    out.innerHTML = '<div class="brief-empty">Connect the AI backend to see monthly picks.</div>';
    return;
  }
...
async function loadVisaMatrix(bucket, force) {
  ...
  if (!CFG.API_URL) { out.innerHTML = '<div class="brief-empty">Connect the AI backend to run visa lookups.</div>'; return; }

  out.innerHTML = boxLoading('Checking current ' + visaBucket.toLowerCase() + ' countries for your passport...');

  try {
    const r = await api('visaMatrix', ctx({ bucket: visaBucket, nocache: force ? Date.now() : '' }));
    const d = r.data || {};
    const list = d.countries || [];
    if (!list.length) throw new Error('Nothing confirmed for that category right now.');
    ...
    out.innerHTML =
      '<div class="visa-grid">' + list.map((c, i) =>
        ...
        '</article>').join('') + '</div>' +
      '<div class="disclaimer">Visa rules change without notice. This list is a starting point, ' +
      'not an authority — open the official source below and confirm before booking.</div>' +
      '<div class="source-list" id="vmSources"></div>';

    renderSources('#vmSources', r.sources, r.checked || d.checked);
    out.querySelectorAll('[data-explore]').forEach(el => {
      el.onclick = () => selectCountry(el.dataset.explore);
      el.addEventListener('keydown', e => {
        if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); el.click(); }
