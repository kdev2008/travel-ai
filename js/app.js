...
async function selectCountry(input, silent) {
  ...
  try {
    ...
    if (!silent) { setScan(10, 'Opening ' + c.name.common + '...'); scanMark('scanGeo'); }
    renderCountry();
    showView('country');

    // Live lightweight utilities can load immediately while the travel matrix is checked.
    await Promise.allSettled([loadRate(), loadWeather()]);
    ...
    const prep = await api('ensureCountry', ctx({
    ...
    if (prep.ready) {
      // Stale data is served instantly; backend refreshes it without making this visitor wait.
      if (!silent) {
        scanMark('scanFx'); scanMark('scanWx'); scanMark('scanVisa'); scanMark('scanAi');
        closeScan(prep.stale ? 'Guide ready · refreshing quietly in the background.' : 'Destination intelligence ready.');
      }
      loadCities();
      loadVisaBrief();
      return;
    ...
    // First-ever search: actively build the destination in small reliable stages.
    // The overlay stays useful and reflects real backend progress.
    ...
    loadVisaBrief();
    loadSafety();
    if (!silent) closeScan(c.name.common + ' travel guide is ready.');
  } catch (e) {
    if (token !== countryToken) return;
...
async function buildCountryProgressively(c, token, silent) {
  let status = await api('countryBuildStatus', { countryCode: c.cca2 });
  let build = status.build || status;
  applyBuildProgress(build);

  // Four stages maximum: CORE, ESSENTIALS, EXPERIENCE_A, EXPERIENCE_B.
  for (let i = 0; i < 5; i++) {
    if (token !== countryToken) return;
    if (String(build.status || '').toUpperCase() === 'READY') return;
    if (String(build.status || '').toUpperCase() === 'ERROR') {
      throw new Error(build.error || 'The destination build stopped. Please try again.');
    }

    // Run the next stage now so the first visitor does not have to wait for the 1-minute trigger.
    const step = await api('processCountryBuild', ctx({ countryCode: c.cca2, passport: profile.passport || profile.home }));
    build = step.build || build;
    applyBuildProgress(build);

    // As soon as CORE exists, populate visible city cards behind the progress panel.
    if (Number(build.progress || 0) >= 42) loadCities();
    if (Number(build.progress || 0) >= 68) {
      loadVisaBrief();
      loadSafety();
    }
    if (String(build.status || '').toUpperCase() === 'READY') return;
  ...
  const latest = await api('countryBuildStatus', { countryCode: c.cca2 });
  build = latest.build || latest;
  applyBuildProgress(build);
  if (String(build.status || '').toUpperCase() !== 'READY') {
    throw new Error(build.error || 'The destination is still preparing. Please try again shortly.');
  }
}
