/* Travel AI — application logic
   BUILD 4.4 — traveller platform, single file (instant static layer + device cache).
...
async function selectCountry(input, silent) {
  ...
  try {
    ...
    // 1) Instant: this device's saved copy, else the published static copy (GitHub Pages CDN).
    const instant = await openInstantBundle(c, token);
    if (token !== countryToken) return;
    if (instant) {
      ensureVisaShown();
      if (!silent) { scanMark('scanAi'); closeScan('Saved guide opened instantly.'); }
      if (CFG.API_URL) revalidateCountry(c, token);   // background: newer data, visit count, refresh queue
      return;
    }
    ...
    // 2) Backend: database-first request (builds the country if it has never been generated).
    ...
    if (token !== countryToken) return;
    if (br.ready) adoptBundle(br.data);
    else { currentCountryBundle = br.data || null; applyCountryBundleToView(currentCountryBundle); }
    ...
    if (token !== countryToken) return;
    if (latest.ready) adoptBundle(latest.data);
    ensureVisaShown();
...
/* ================= INSTANT LAYER (4.4) =================
   Order: device copy (IndexedDB, 0 ms) -> static JSON on GitHub Pages (~0.1 s)
   -> Apps Script (only when neither exists, or quietly in the background). */
...
const bundleStore = (() => {
  ...
  const open = () => dbp || (dbp = new Promise(res => {
    try {
      ...
      const rq = indexedDB.open('travel-ai', 1);
      rq.onupgradeneeded = () => rq.result.createObjectStore('bundles');
      rq.onsuccess = () => res(rq.result);
      rq.onerror = () => res(null);
      rq.onblocked = () => res(null);
    } catch (e) { res(null); }
  }));
  // A slow open (cold start, private mode) must never block the page, but it must not
  // disable the cache either: each read gives up after 1.5 s, the connection stays usable.
  ...
  const run = async (mode, fn) => {
    ...
    return new Promise(res => {
      try {
        const rq = fn(db.transaction('bundles', mode).objectStore('bundles'));
  ...
  return {
    get: code => withTimeout(run('readonly', st => st.get(code)), 1500),
    put: (code, val) => run('readwrite', st => st.put(val, code))
  };
})();
const localStaticV = {};                 // country code -> static version stored on this device

function staticBase() { return String(CFG.STATIC_DATA_URL || 'data/countries/').replace(/\/?$/, '/'); }
let staticIndexP = null, staticIndexAt = 0;
function staticIndex() {
  if (!staticIndexP || Date.now() - staticIndexAt > 10 * 60 * 1000) {
    staticIndexAt = Date.now();
    staticIndexP = fetchTimeout(staticBase() + 'index.json?t=' + Math.floor(Date.now() / 300000), { cache: 'no-cache' }, 4000)
      .then(r => (r && r.ok ? r.json() : null)).catch(() => null);
  }
  return staticIndexP;
}
async function staticEntry(code) {
  const idx = await staticIndex();
  return (idx && idx.countries && idx.countries[code]) || null;
}
async function fetchStaticBundle(code, v) {
  try {
    const r = await fetchTimeout(staticBase() + code + '.json?v=' + encodeURIComponent(v || ''), {}, 6000);
    return r && r.ok ? await r.json() : null;
  } catch (e) { return null; }
}

function usableBundle(b) { return !!(b && b.country && Array.isArray(b.cities) && b.cities.length && b.months); }
function bundleCode(b) { return String((b && b.country && b.country.Code) || '').toUpperCase(); }
function bundleFreshness(b) {
  const t = (b && b.sectionTimes) || {};
  return [b && b.updatedAt].concat(Object.keys(t).map(k => t[k] && t[k].updatedAt)).filter(Boolean).sort().pop() || '';
}
function bundleSig(b) {
  if (!b) return '';
  return JSON.stringify([bundleCode(b), b.updatedAt, b.sectionTimes, Object.keys(b.visas || {}).sort(), !!b.cityDetails]);
}
function saveLocal(b, v) {
  const code = bundleCode(b);
  if (!code || !usableBundle(b)) return;
  if (v) localStaticV[code] = v;
  bundleStore.put(code, { bundle: b, v: localStaticV[code] || '', savedAt: Date.now() });
}

/* Make a bundle current: keep device-only extras (dossiers, visas learned here), re-render only if something changed, save it. */
function adoptBundle(nb, v) {
  if (!nb) return;
  const old = currentCountryBundle;
  const same = old && bundleCode(old) === bundleCode(nb);
  if (same && !nb.cityDetails && old.cityDetails) {
    const dt = b => (b.sectionTimes && b.sectionTimes.cityDetails && b.sectionTimes.cityDetails.updatedAt) || '';
    if (dt(old) === dt(nb)) nb.cityDetails = old.cityDetails;
  }
  if (same && old.visas) nb.visas = Object.assign({}, old.visas, nb.visas || {});
  const changed = bundleSig(nb) !== bundleSig(old);
  currentCountryBundle = nb;
  if (changed) applyCountryBundleToView(nb);
  saveLocal(nb, v);
}

async function openInstantBundle(c, token) {
  const code = c.cca2;
  const local = await bundleStore.get(code);
  if (token !== countryToken) return null;
  if (local && usableBundle(local.bundle)) {
    localStaticV[code] = local.v || '';
    currentCountryBundle = null;
    adoptBundle(local.bundle);
    refreshFromStatic(code, token);           // newer published copy? swap in quietly
    return local;
  }
  const entry = await staticEntry(code);
  if (!entry || token !== countryToken) return null;
  const st = await fetchStaticBundle(code, entry.v);
  if (token !== countryToken || !usableBundle(st)) return null;
  currentCountryBundle = null;
  adoptBundle(st, entry.v);
  return { bundle: st, v: entry.v };
}
async function refreshFromStatic(code, token) {
  const entry = await staticEntry(code);
  if (!entry || entry.v === localStaticV[code] || token !== countryToken) return;
  const st = await fetchStaticBundle(code, entry.v);
  if (token !== countryToken || !usableBundle(st)) return;
  if (bundleFreshness(st) >= bundleFreshness(currentCountryBundle)) adoptBundle(st, entry.v);
  else localStaticV[code] = entry.v;
}

/* Background check against the live database: newer sections, visit count, refresh queue. */
const lastRevalidate = {};
async function revalidateCountry(c, token) {
  const code = c.cca2;
  if (Date.now() - (lastRevalidate[code] || 0) < 60000) return;
  lastRevalidate[code] = Date.now();
  try {
    const r = await api('countryBundle', ctx({ country: c.name.common, countryCode: code, passport: profile.passport || profile.home }));
    if (token !== countryToken) return;
    if (r.ready && r.data) { adoptBundle(r.data); ensureVisaShown(); }
    if (needsWatch(r)) watchCountryBundle(c, token);
  } catch (e) { /* the saved copy is already on screen */ }
}

/* City dossier from the published copy -> same shape the cityExplore endpoint returns. */
function localDossier(city) {
  const d = currentCountryBundle && currentCountryBundle.cityDetails;
  return d ? d[String(city || '').trim().toLowerCase()] || null : null;
}
async function usdTo(cur) {
  cur = String(cur || 'USD').toUpperCase();
  return cur === 'USD' ? 1 : (await getRate('USD', cur)).rate;
}
async function dossierView(x, local) {
  const fx = await usdTo(local);
  const mid = Number((x.dailyUSD || {})['Mid-range'] || 0);
  const amt = v => (v == null || v === '' ? null : Math.round(Number(v) * fx));
  return {
    currency: local, intro: x.intro || '', score: x.score || 0, days: x.days || '', bestTime: x.bestTime || '',
    gettingAround: x.gettingAround || '', perfectFor: x.perfectFor || [],
    daily: { stay: Math.round(mid * .42 * fx), food: Math.round(mid * .24 * fx), transport: Math.round(mid * .12 * fx), activities: Math.round(mid * .22 * fx) },
    sights: (x.sights || []).map(a => ({ name: a.name, category: a.category, what: a.what, why: a.why, fee: { amount: amt(a.feeUSD), currency: local, note: 'Database reference' }, duration: a.duration, bestTime: a.bestTime })),
    activities: (x.activities || []).map(a => ({ name: a.name, category: a.category, what: a.what, price: { amount: amt(a.priceUSD), currency: local, note: 'Database reference' }, duration: a.duration, bestFor: a.bestFor })),
    food: x.food || [], tips: x.tips || []
  };
}
/* Cost tiers from the saved country row -> same shape the tripCosts endpoint returns. */
async function localCostTiers() {
  const c = currentCountryBundle && currentCountryBundle.country;
  if (!c) return null;
  const values = [['Budget', c.DailyBudgetUSD], ['Mid-range', c.DailyMidUSD], ['Comfort', c.DailyComfortUSD], ['Luxury', c.DailyLuxuryUSD]];
  if (!values.some(v => Number(v[1]) > 0)) return null;
  const cur = profile.currency || 'USD';
  const fx = await usdTo(cur);
  return {
    currency: cur, days: 7, checked: c.UpdatedAt || '',
    tiers: values.map(([tier, usd]) => {
      const d = Number(usd || 0) * fx;
      return { tier, stay: Math.round(d * .42), food: Math.round(d * .24), transport: Math.round(d * .12), activities: Math.round(d * .22), flightLow: null, flightHigh: null };
    })
  };
}
...
async function watchCountryBundle(c, token) {
  for (let i = 0; i < 12; i++) {
    ...
    if (token !== countryToken) return;
    if (r.data && r.ready) adoptBundle(r.data);        // re-renders only when something new landed
...
function applyBuildProgress(build) {
  ...
  const messages = {
    ...
    DETAILS: 'Checking visa & safety and writing city guides in parallel...',
    EXPERIENCE: 'Writing city guides and local experiences...',
...
async function buildCountryProgressively(c, token, silent) {
  ...
  // Stages: CORE, then DETAILS (visa/safety + city guides in parallel). Time-boxed rather than
...
async function openCity(city) {
  ...
  try {
    let r = null;
    const dossier = localDossier(city);
    if (dossier) {
      try { r = { data: await dossierView(dossier, localCur), sources: [], checked: '' }; }
      catch (e) { r = null; }                      // FX unavailable: ask the backend instead
    }
    if (!r) r = await api('cityExplore', ctx({
...
async function loadCosts(force) {
  ...
  try {
    let r = null;
    if (!force) {
      try { const local = await localCostTiers(); if (local) r = { data: local, sources: [], checked: local.checked }; }
      catch (e) { r = null; }                      // FX unavailable: ask the backend instead
    }
    if (!r) r = await api('tripCosts', ctx({
      country: selectedCountry.name.common,
      countryCode: selectedCode,
      days: 7,
      nocache: force ? Date.now() : ''
...
async function loadVisaBrief(force) {
  ...
  const flightKey = selectedCode + ':' + pk;
  if (!force && visaInflight === flightKey) return;
  visaInflight = flightKey;
  ...
  try {
    ...
    if (visaInflight === flightKey) visaInflight = '';
    if (token !== countryToken) return;
    if (r.data && r.data.status && currentCountryBundle && bundleCode(currentCountryBundle) === selectedCode) {
      currentCountryBundle.visas = currentCountryBundle.visas || {};
      currentCountryBundle.visas[pk] = { status: r.data.status, note: r.data.note || '', maxStay: r.data.maxStay || '', updatedAt: r.checked || '' };
      saveLocal(currentCountryBundle);            // next visit shows this visa instantly
    }
