...
const COUNTRY_FIELDS = 'name,cca2,cca3,flag,capital,capitalInfo,currencies,languages,population,timezones,idd,car,region,subregion,continents,area,latlng';

async function restCountries(path, useFields) {
  const base = CFG.REST_COUNTRIES_BASE || 'https://restcountries.com/v3.1';
  const url = base + path + (useFields ? (path.indexOf('?') > -1 ? '&' : '?') + 'fields=' + COUNTRY_FIELDS : '');
...
async function lookupByName(q) {
  try { return await restCountries('/name/' + encodeURIComponent(q), true); }
  catch (e) {
    try { return await restCountries('/name/' + encodeURIComponent(q), false); }
    catch (e2) { return []; }
  }
}
...
async function fetchCountryByCode(code) {
  ...
  let list = [];
  try { list = await restCountries('/alpha/' + encodeURIComponent(key), true); }
  catch (e) {
    try { list = await restCountries('/alpha/' + encodeURIComponent(key), false); }
    catch (e2) { list = []; }
  }

  const c = list[0];
  if (!c || !c.cca2) throw new Error('Country data is unavailable right now. Try again in a moment.');
  countryCache.set(c.cca2.toUpperCase(), c);
  return c;
}

async function resolveCountry(input) {
  const q = normalizeQuery(input);
  if (!q) throw new Error('Type a country name to begin.');
  ...
  if (/^[a-z]{3}$/i.test(q)) {
    try {
      const list = await restCountries('/alpha/' + encodeURIComponent(q), true);
      if (list[0] && list[0].cca2) return list[0].cca2;
    } catch (e) { /* fall through to name search */ }
  }

  const list = await lookupByName(q);
  if (!list.length) throw new Error('No country matches "' + q + '".');

  const exact = list.find(c =>
    String((c.name && c.name.common) || '').toLowerCase() === low ||
    String((c.name && c.name.official) || '').toLowerCase() === low
  );

  const startsWith = list.find(c =>
    String((c.name && c.name.common) || '').toLowerCase().indexOf(low) === 0
  );

  const best = exact || startsWith || list.slice().sort((a, b) =>
    String((a.name && a.name.common) || '').length - String((b.name && b.name.common) || '').length
  )[0];

  if (!best || !best.cca2) throw new Error('No country matches "' + q + '".');
  countryCache.set(best.cca2.toUpperCase(), best);
  return best.cca2;
}
...
async function searchCountries(q) {
  ...
  const low = q.toLowerCase();
  if (aliases[low]) {
    try { return [await fetchCountryByCode(aliases[low])]; } catch (e) { /* continue */ }
  }
  if (/^[a-z]{2}$/i.test(q)) {
    try { return [await fetchCountryByCode(q)]; } catch (e) { /* continue */ }
  }
  ...
  return list
    .slice()
    .sort((a, b) => {
      const an = String((a.name && a.name.common) || '').toLowerCase();
      const bn = String((b.name && b.name.common) || '').toLowerCase();
      const ai = an.indexOf(low) === 0 ? 0 : 1;
      const bi = bn.indexOf(low) === 0 ? 0 : 1;
...
function bindCountrySearch(inputSelector, resultSelector, buttonSelector) {
  ...
  input.addEventListener('input', () => {
    ...
    timer = setTimeout(async () => {
      let list = [];
      try { list = await searchCountries(q); } catch (e) { list = []; }
...
function renderCountry() {
  ...
  setText('#timezone', (c.timezones || ['—'])[0]);
  setText('#callingCode', ((c.idd && c.idd.root) || '') + (((c.idd && c.idd.suffixes) || [''])[0] || '') || '—');
...
async function loadWeather() {
  ...
  const ci = selectedCountry.capitalInfo || {};
  const lat = (ci.latlng && ci.latlng[0]) ?? (selectedCountry.latlng || [])[0];
  const lon = (ci.latlng && ci.latlng[1]) ?? (selectedCountry.latlng || [])[1];
  ...
  if (lat == null || lon == null) { fail('No capital coordinates'); return; }
