/* Travel AI — application logic
   BUILD 2.6 — single-file. The country dataset is embedded below, so there is no
   separate data file to upload and nothing external to fetch for country facts. */
...
/* ---------- EMBEDDED COUNTRY DATASET (250 territories) ----------
   Sources: mledoze/countries (MPL-2.0), dr5hn/countries-states-cities-database (ODbL). */
...
const DATASET_MISSING = 'The country dataset is empty. js/app.js did not load completely.';
...
async function selectCountry(input, source, silent) {
  ...
  try {
    ...
    if (silent) {
      ...
      setScan(100, (e.message || 'That destination could not be loaded.') + '  ·  build ' + BUILD);
...
function openTool(action) {
  ...
  if (action === 'tools') {
    const tools = [
      ['planner', '🧭', 'Trip planner', 'Short day-by-day plan'],
      ['budget', '💳', 'Budget estimate', 'Simple cost bands'],
      ['visa', '🛂', 'Visa research', 'Answer plus official links'],
      ['safety', '🛡', 'Safety brief', 'Precautions plus sources'],
      ['compare', '⚖', 'Compare countries', 'Side by side'],
      ['currency', '💱', 'Currency converter', 'Reference rates'],
      ['checklist', '☑', 'Packing checklist', 'Saved on this device'],
      ['profile', '👤', 'Traveller profile', 'Passport and currency']
    ];
    openModal('<h2>Tools</h2><div class="tool-grid sheet-tools">' + tools.map(t =>
      '<article data-tool="' + t[0] + '" role="button" tabindex="0"><span aria-hidden="true">' + t[1] +
      '</span><b>' + esc(t[2]) + '</b><small>' + esc(t[3]) + '</small></article>').join('') + '</div>');
    $$('[data-tool]').forEach(el => {
      el.onclick = () => { closeModal(); setTimeout(() => openTool(el.dataset.tool), 180); };
...
function toggleAiSheet() {
  if (document.body.classList.contains('ai-open')) closeAiSheet();
  else openAiSheet();
}
...
on('#aiSheetClose', closeAiSheet);

/* Tapping outside the assistant closes it. */
document.addEventListener('click', e => {
  if (!document.body.classList.contains('ai-open')) return;
  if (e.target.closest('#aiPanel') || e.target.closest('#tabbar')) return;
  closeAiSheet();
});

/* Escape closes the assistant as well as modals. */
document.addEventListener('keydown', e => {
  if (e.key === 'Escape' && document.body.classList.contains('ai-open')) closeAiSheet();
});

/* Asking a question on a phone should surface the answer. */
const _askAI = askAI;
askAI = function (q) {
  if (isPhone() && normalizeQuery(q)) openAiSheet();
  return _askAI(q);
};

/* When a modal closes, hand the highlight back to Explore. */
const _closeModal = closeModal;
closeModal = function () {
  _closeModal();
  if (!document.body.classList.contains('ai-open')) setNav('explore');
};
...
window.addEventListener('beforeinstallprompt', e => {
  ...
  installEvent = e;
  let dismissed = false;
  try { dismissed = localStorage.getItem('travel_ai_install_hidden') === '1'; } catch (err) { /* ignore */ }
...
/* ---------- SELF TEST ----------
   Open the site with #diag on the end of the URL, or run travelAIDiag() in the
   browser console, to see exactly what is and is not working. */

async function travelAIDiag() {
  const rows = [];
  const add = (label, ok, detail) => rows.push(
    '<div class="fact-row"><span>' + esc(label) + '</span><b>' +
    (ok ? '✓ ' : '✗ ') + esc(detail) + '</b></div>');

  add('Build', true, BUILD);
  add('Country dataset', COUNTRIES.length === 250, COUNTRIES.length + ' loaded');

  for (const q of ['Japan', 'USA', 'Thai']) {
    try {
      const code = await resolveCountry(q);
      const c = await fetchCountryByCode(code);
      add('Resolve "' + q + '"', true, c.flag + ' ' + c.name.common);
    } catch (e) { add('Resolve "' + q + '"', false, e.message); }
  }

  add('Backend URL set', !!CFG.API_URL, CFG.API_URL ? 'configured' : 'API_URL is empty in js/config.js');

  openModal('<h2>Travel AI diagnostics</h2><div id="diagRows">' + rows.join('') +
    '<div class="fact-row"><span>Exchange rate</span><b id="dFx">checking...</b></div>' +
    '<div class="fact-row"><span>Weather service</span><b id="dWx">checking...</b></div>' +
    '<div class="fact-row"><span>AI backend</span><b id="dApi">checking...</b></div>' +
    '</div><p class="brief-copy">Screenshot this panel if you need to report a problem.</p>');

  getRate('INR', 'JPY')
    .then(r => setText('#dFx', '✓ 1 INR = ' + formatRate(r.rate) + ' JPY'))
    .catch(e => setText('#dFx', '✗ ' + e.message));

  fetchWithTimeout((CFG.OPEN_METEO_FORECAST || '') + '?latitude=28.6&longitude=77.2&current=temperature_2m', {}, 10000)
    .then(r => r.json())
    .then(j => setText('#dWx', '✓ ' + Math.round(j.current.temperature_2m) + '°C in New Delhi'))
    .catch(e => setText('#dWx', '✗ unreachable'));

  if (CFG.API_URL) {
    api('health')
      .then(j => setText('#dApi', '✓ v' + j.version + (j.keyConfigured ? ', key set' : ', NO API KEY')))
      .catch(e => setText('#dApi', '✗ ' + e.message));
  } else {
    setText('#dApi', '— not configured');
  }
}
...
console.log('%cTravel AI build ' + BUILD + ' — ' + COUNTRIES.length +
  ' countries loaded. Run travelAIDiag() for a self test.',
  'color:#3ad;font-weight:bold');
...
if (location.hash === '#ai') setTimeout(openAiSheet, 500);
if (location.hash === '#trips') setTimeout(() => openTool('saved'), 500);
